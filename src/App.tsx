...
export default function App() {
  ...
  // Check if URL has mode=readonly or presentation=true or view=presentation
  const isReadOnlyMode = typeof window !== 'undefined' && (
  ...
  const [isPresentationMode, setIsPresentationMode] = useState<boolean>(() => {
    if (typeof window !== 'undefined') {
      ...
      return params.get('mode') === 'readonly' || params.get('view') === 'presentation' || params.get('presentation') === 'true';
    }
    return false;
  });
    ...
    <div className="min-h-screen bg-slate-950 text-slate-200 font-sans flex flex-col antialiased select-none border border-slate-800">
      ...
      <header className="border-b border-slate-800 bg-slate-900 px-6 py-4 flex items-center justify-between shrink-0">
        ...
        <div className="flex items-center gap-3">
          ...
          {isReadOnlyMode ? (
            <div className="bg-amber-500/10 text-amber-400 border border-amber-500/20 px-4 py-2 rounded-lg text-sm font-semibold flex items-center gap-2 font-mono shadow-md">
              <span className="w-2 h-2 rounded-full bg-amber-400 animate-pulse"></span>
              <span>EXIBIÇÃO (LEITURA)</span>
            </div>
          ) : (
            <button
              ...
              filteredHistory.map((item) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex gap-2 pt-1.5 border-t border-slate-800/60 items-center justify-between">
                      <div className="flex items-center gap-1.5 text-slate-500 font-mono text-[10px]">
                        <ListTodo className="w-3 h-3 text-slate-500" />
                        <span>{(item.actions5w2h || []).length} ações</span>
                      </div>
                      {item.incidents > 0 ? (
                        <div className="flex items-center gap-1">
                          ...
                          <span className="text-[10px] font-mono text-rose-400 font-bold bg-rose-950/40 px-1 py-0.2 rounded border border-rose-500/20">
                            {item.incidents} desvio{item.incidents > 1 ? 's' : ''}
                          </span>
                        </div>
                      ) : (
                        <span className="text-[10px] font-mono text-emerald-500 font-medium">Planta Segura ✓</span>
                      )}
                    </div>
                  </button>
                );
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2 mt-1 flex-wrap">
                        {history.length > 0 ? (
                          <div className="relative inline-block max-w-[220px]">
                            <select
                              ...
                              className="w-full bg-slate-950 font-display font-medium text-gray-200 text-[13px] sm:text-xs hover:text-white border border-[#232938] hover:border-slate-750 rounded-lg pl-3 pr-8 py-1 focus:outline-none transition-all cursor-pointer appearance-none truncate"
                            >
                              {history.map((h) => (
                                <option key={h.id} value={h.id} className="bg-slate-950 text-slate-300">
                                  {h.weekName} ({h.date.split('-').reverse().slice(0, 2).join('/')})
                                </option>
                              ))}
                            </select>
                        ...
                        {/* Quick arrows to switch meetings quickly */}
                        {history.length > 1 && (
                          <div className="flex items-center bg-slate-950 border border-[#232938] rounded-lg p-0.5 shadow-inner">
                            <button
                              type="button"
                              onClick={() => {
                                const currentIndex = history.findIndex(h => h.id === selectedMeeting?.id);
                                if (currentIndex > 0) {
                                  handleSelectMeeting(history[currentIndex - 1]);
                                } else {
                                  handleSelectMeeting(history[history.length - 1]);
                                }
                              }}
                              title="Reunião Anterior no Histórico"
                              className="cursor-pointer p-0.5 text-slate-400 hover:text-cyan-400 hover:bg-slate-900 rounded transition"
                            >
                              <ChevronLeft className="w-3.5 h-3.5" />
                            </button>
                            <span className="text-[9px] font-mono font-bold text-slate-500 px-1.5 border-x border-slate-900">
                              {history.findIndex(h => h.id === selectedMeeting?.id) + 1}/{history.length}
                            </span>
                            <button
                              type="button"
                              onClick={() => {
                                ...
                                if (currentIndex < history.length - 1) {
                                  ...
                                  handleSelectMeeting(history[0]);
                                }
                              }}
                              title="Próxima Reunião no Histórico"
                              className="cursor-pointer p-0.5 text-slate-400 hover:text-cyan-400 hover:bg-slate-900 rounded transition"
                            >
                              ...
                              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-left">
                                <div className="bg-[#121620] border border-[#232938] p-5 rounded-xl shadow-lg flex flex-col gap-1">
                                  ...
                                  <div className="flex items-baseline gap-1.5 mt-1">
                                    ...
                                    <span className="text-xs text-slate-550">Sincros</span>
                                  </div>
                                  <span className="text-[10px] text-slate-505 mt-2 flex items-center gap-1.5 font-bold">
                                    <Clock className="w-3.5 h-3.5 text-blue-400" /> Registros salvos no histórico
                                  </span>
                                </div>

                                <div className="bg-[#121620] border border-[#232938] p-5 rounded-xl shadow-lg flex flex-col gap-1">
                                  <span className="text-[10px] text-slate-450 font-mono tracking-widest uppercase font-bold">Total de Ocorrências</span>
                                  <div className="flex items-baseline gap-1.5 mt-1">
                                    <span className={`text-3xl font-mono font-bold ${grandTotalIncidents > 0 ? 'text-rose-400' : 'text-emerald-400'}`}>{grandTotalIncidents}</span>
                                    <span className="text-xs text-slate-505">Desvios</span>
                                  </div>
                                  <span className="text-[10px] text-slate-505 mt-2 flex items-center gap-1.5 font-bold">
                                    <ShieldCheck className="w-3.5 h-3.5 text-emerald-400" /> Indicador cumulativo de segurança
                                  </span>
                                </div>
                              </div>
                                ...
                                <div className="bg-[#121620] border border-[#232938] rounded-xl p-5 shadow-lg space-y-4">
                                  ...
                                  <div className="space-y-4">
                                    {Object.entries(shiftMetrics).map(([sName, m]) => {
                                      return (
                                        <div key={sName} className="p-4 bg-slate-950/40 border border-[#232938] rounded-xl flex items-center justify-between gap-4">
                                          <div>
                                            <div className="font-display font-bold text-white text-base">{sName}</div>
                                            <div className="text-[10.5px] font-mono text-slate-500 uppercase mt-0.5">{m.count} reuniões integradas</div>
                                          </div>
                                          <div className="bg-slate-900/40 border border-[#232938] p-3 rounded-lg text-center min-w-[120px]">
                                            <div className="text-[9px] uppercase font-bold tracking-wider text-slate-400 block mb-0.5">Incidentes</div>
                                            <span className={`text-xl font-mono font-bold ${m.incidents > 0 ? 'text-rose-400' : 'text-emerald-450'}`}>
                                              {m.incidents > 0 ? `🚨 ${m.incidents}` : '🟢 Zero'}
                                            </span>
                                          </div>
                                        </div>
                                      );
                          ...
                          {analyticsSubTab === 'team' && (
                            <div className="space-y-6 animate-fadeIn">
                              <div className="grid grid-cols-1 sm:grid-cols-4 gap-4 text-left font-mono">
                                ...
                                <div className="bg-[#121620] border border-[#232938] p-5 rounded-xl shadow-lg flex flex-col gap-1">
                                  <span className="text-[10px] text-slate-400 font-mono tracking-widest uppercase font-bold">Fera dos Reconhecimentos</span>
                                  <div className="flex items-baseline gap-1.5 mt-1">
                        ...
                        <div className="space-y-6 animate-fadeIn">
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            ...
                            <div className="bg-slate-900 border border-slate-800 p-5 rounded-xl shadow-lg flex flex-col gap-1 hover:border-rose-500/30 transition">
                              <span className="text-[10px] text-slate-500 font-mono tracking-widest uppercase font-bold">Total de Ocorrências</span>
                              <div className="flex items-baseline gap-1.5 mt-1">
                                <span className={`text-3xl font-mono font-bold ${grandTotalIncidents > 0 ? 'text-rose-400' : 'text-emerald-400'}`}>{grandTotalIncidents}</span>
                                <span className="text-xs text-slate-505">Desvios</span>
                              </div>
                          ...
                          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                            ...
                            <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 shadow-lg space-y-4">
                              ...
                              <div className="space-y-4">
                                {Object.entries(shiftMetrics).map(([sName, m]) => {
                                  return (
                                    <div key={sName} className="p-4 bg-slate-950/40 border border-slate-800 rounded-xl flex items-center justify-between gap-4">
                                      <div>
                                        <div className="font-display font-bold text-white text-base">{sName}</div>
                                        <div className="text-[10.5px] font-mono text-slate-500 uppercase mt-0.5">{m.count} reuniões integradas</div>
                                      </div>
                                      <div className="bg-slate-900/40 border border-slate-800 p-3 rounded-lg text-center min-w-[120px]">
                                        <div className="text-[9px] uppercase font-bold tracking-wider text-slate-400 block mb-0.5">Incidentes</div>
                                        <span className={`text-xl font-mono font-bold ${m.incidents > 0 ? 'text-rose-400' : 'text-emerald-400'}`}>
                                          {m.incidents > 0 ? `🚨 ${m.incidents}` : '🟢 Zero'}
                                        </span>
                                      </div>
                                    </div>
